...
from models.goha_wrapper import GOHAWrapper
...
class EncodeTester():
    ...
    def test(self, tar_fnames, tar_mask_fnames, save_tri=True):
        ...
        for i in tqdm(range(self.frame_limit)):
            ...
            tar_img_masked = torch.from_numpy(tar_img_masked[None, ...]).to(torch.float32) / 127.5 - 1

            start = timer()
            expr_planes = self.model(tar_img_masked) 
            end = timer()
