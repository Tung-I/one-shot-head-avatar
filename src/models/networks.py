...
class IDUpsampler(torch.nn.Module):
    """
    Upsample ID plane from 256x128x128 to 256x256x256.
    """
    ...
    def forward(self, id_planes):
        """ 
        To Do: potential bug, all self.up_xy?
        """
...
class OSGDecoder(torch.nn.Module):
    """
    Decoder for the tri-plane features.
    """
...
class Embed2Plane(torch.nn.Module):
    """
    Spatially upsample embedding from 256x128x128 to 96x256x256 tri-planes.
    """
    def __init__(self, in_dim, out_dim):
        super(Embed2Plane, self).__init__()
        self.up_layers = nn.Sequential(nn.Conv2d(in_dim, 256, 3, 1, 1), # 256x128x128
                                       nn.LeakyReLU(0.2),
                                       ...
                                       nn.LeakyReLU(0.2),
                                       nn.Conv2d(128, out_dim, 3, 1, 1)) # 96x256x256
